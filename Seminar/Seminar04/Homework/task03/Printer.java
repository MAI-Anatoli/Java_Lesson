...
// 3.0
// 6.0 */
package Seminar.Seminar04.Homework.task03;

import java.util.ArrayDeque;
import java.util.Deque;

class Calculator {
    private Deque<Double> results; // стек для хранения результатов операций
    ...
    // Дополнение метода для отмены последней операции
    public double calculate(char op, double a, double b, char undo) {
        if (op == '<' && !results.isEmpty() && results.size() >= 2) {
            results.pop(); // удаляем последний результат
            return results.peek(); // возвращаем предпоследний результат
        } else {
            return calculate(op, a, b); // возвращаем результат основного метода
        }
    }
}
...
public class Printer {
    public static void main(String[] args) {
        ...
        double prevResult = calculator.calculate(undo, 0, 0, undo);
